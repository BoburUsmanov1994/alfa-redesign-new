...
const DistributionTypePage = () => {
    ...
    const create = (attrs) => {
        mutate({url: URLS.distributeType, attributes: {...attrs}}, {
            onSuccess: () => {
                setOpen(false)
                ref.current?.reload()
            }
    ...
    const update = (attrs) => {
        updateRequest({
            url:`${URLS.distributeType}/${get(currentRecord,'_id')}`,attributes: {...attrs}
        },{
            onSuccess:()=>{
    ...
    const remove = (_id) => {
        removeRequest({url:`${URLS.distributeType}/${_id}`},{
            onSuccess:()=>{
                ref.current?.reload()
            }
        })
        ...
        <>
            ...
            <Datagrid
                ...
                columns={[
                    ...
                    {
                        title: t('Код дебиторки'),
                        dataIndex: 'debt_account',
                        render:(_,record)=>get(record,'debt_account.account_ID'),
                        hideInSearch: true,
                        align:'center'
                    },
                    {
                        title: t('Код кредиторки'),
                        dataIndex: 'cred_account',
                        render:(_,record)=>get(record,'cred_account.account_ID'),
                        hideInSearch: true,
                        align:'center'
                    },
                    {
